...
@Composable
fun FavoriteScreen(dataModel: DataViewModel = viewModel()) {
    val favoriteMeals by dataModel.favoriteMeals.observeAsState(emptyList())
...
@Composable
fun FavoriteMealItem(meal: Meal) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .height(81.dp)
...
fun EmptyFavoriteScreen() {
    Column(
        ...
        Text(
            ...
            color = colorResource(id = R.color.featuredMealName),
            fontSize = 16.sp,
            modifier = Modifier.width(253.dp).align(Alignment.CenterHorizontally)
        )
